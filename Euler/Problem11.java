...
public class Problem11{
	public static void main(String [] args){
		...
		int x=0,y=0;
		//put all numbers into a 2d array
		for(int i=0;i<nums.length();i=i+3){
			grid[x][y]=Integer.parseInt(nums.substring(i,i+2));
			if(y==19){
				x++;
				y=0;
			}else{
		...
		int total=0,max=0;
		//check the array for highest product of four connected elements
		for(int i=0;i<20;i++){
			for(int j=0;j<20;j++){
				if(i<17){
